...
**メリット:**
- ✅ 2系のボリュームゾーンをカバー（2.3.x～2.6.x）
- ✅ 現在の実装で完全に動作する（ProviderObserver API は2系全体で同じ）
- ✅ StreamNotifier 導入後の安定版以降
- ✅ 無駄に古いバージョンを含まない

**カバー範囲:**
- Version 2.3.x～2.6.x: 主流ユーザー（2系の大部分）
- Version 3.x: 全ユーザー対応（最新）

**対象ユーザー:** 2023年以降のプロジェクト（推定80-90%のユーザー）
...
これにより、現在の `>=2.6.1` から `>=2.3.0` に拡張でき、2系のボリュームゾーンをカバーできます。

## テスト推奨事項

最小バージョンを下げる場合、以下のバージョンでテストすることを推奨：
1. `flutter_riverpod: 2.3.0` - 最小バージョン
2. `flutter_riverpod: 2.4.10` - 2.4.x の安定版
3. `flutter_riverpod: 2.5.1` - 2.5.x の安定版
4. `flutter_riverpod: 2.6.1` - 2.x の最終版
